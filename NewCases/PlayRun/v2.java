...
public class PlayRun extends ConventionTask {
    ...
    public void run() {
        ...
        if (!deploymentHandle.isRunning()) {
            ...
            try {
                ...
                PlayRunSpec spec = new DefaultPlayRunSpec(runtimeClasspath, changingClasspath, applicationJar,
                        assetsJar, assetsDirs, getProject().getProjectDir(), getForkOptions(), httpPort);
     ...
     *
     * This port should be available. The Play application will fail to start if the
    ...
    /**
     * The directories of the assets for the Play application (for live reload
     * functionality).
     */
    ...
    private PlayApplicationDeploymentHandle registerOrFindDeploymentHandle(String deploymentId) {
        ...
        PlayApplicationDeploymentHandle deploymentHandle = deploymentRegistry.get(PlayApplicationDeploymentHandle.class,
                deploymentId);
...
// http://refactoring.encs.concordia.ca/oracle/refactoring-details/1156456/6120